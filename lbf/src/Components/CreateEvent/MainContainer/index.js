...
function CreateEventContainer() {
  ...
  function handleFieldsCreateChange(e) {
    ...
    if (e.target.name === "dateTo" || e.target.name === "dateFrom") {
      ...
      const newTagsAdded = allEvents.find(
        (events) => events.name === e.target.value
      );
      setSelectedEventTags([...selectedEventTags, newTagsAdded]);
    } else if (e.target.name === "language" && e.target.value !== null) {
      const newLanguageAdded = allLanguages.find(
  ...
  const handleClickClosedLanguage = (language) => {
    setSelectedLanguages(
      selectedLanguages.filter(
        (selectedlanguage) => selectedlanguage.name !== language.name
      )
    );
  };
  ...
  const handleClickClosedEvents = (events) => {
    setSelectedEventTags(
      selectedEventTags.filter(
        (selectedEventTags) => selectedEventTags.name !== events.name
      )
    );
  };
  ...
  const uploadImage = (e) => {
    ...
    axios
      .post("https://api.cloudinary.com/v1_1/lbfcloud/image/upload", formData)
              ...
              <form id="registerForm" onSubmit={handleSubmitForm}>
                <div className="globale">
                  ...
                  <div className="first">
                    ...
                    <div className="searchEvent__container-infosDetails-location__tag-selected">
                      {selectedEventTags.map((events) => (
                        <Tag
                          handleClick={() => handleClickClosedEvents(events)}
                          name={events.name}
                        />
                      ))}
                    </div>
                    ...
                    <div className="searchEvent__container-infosDetails-location__tag-selected">
                      {selectedLanguages.map((language) => (
                        <Tag
                          handleClick={() =>
                            handleClickClosedLanguage(language)
                          }
                          name={language.name}
                        />
                      ))}
                  ...
                  <div className="second">
                    ...
                    <input
                      className="createEvent__container-eventTitle-uploadInput"
                      type="file"
