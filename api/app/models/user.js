...
class User extends CoreModel {
    ...
    static async validByEmailPassword(email, password) {
        try {
            const { rows } = await db.query(`SELECT "user".id, "user".firstname, "user".lastname, "user".gender, "user".email, "user".password, "user".description AS description, "user".age, "user".city, "user".phone_number AS "phoneNumber", "user".img_url AS "img_url", "user".created_at AS "createdAt", "user".updated_at AS "updatedAt",
            json_agg(
                DISTINCT jsonb_strip_nulls(
                    jsonb_build_object(
                        'id', speaking_language.id,
